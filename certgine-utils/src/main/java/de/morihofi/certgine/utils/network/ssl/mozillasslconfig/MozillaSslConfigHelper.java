package de.morihofi.certgine.utils.network.ssl.mozillasslconfig;
...
public class MozillaSslConfigHelper {
    ...
    private static BasicConfiguration parseGuidelineJson(@NonNull String json, @NonNull CONFIGURATION configuration) {
        ...
        de.morihofi.certgine.utils.network.ssl.mozillasslconfig.response.version5dot1up.Configuration mozCfg = mapConfiguration(configuration, resp);
    ...
    @NonNull
    private static de.morihofi.certgine.utils.network.ssl.mozillasslconfig.response.version5dot1up.Configuration mapConfiguration(@NonNull CONFIGURATION type, @NonNull MozillaSslConfiguration5dot1upResponse resp) {
        return switch (type) {
            case MODERN -> resp.getConfigurations().getModern();
            case INTERMEDIATE -> resp.getConfigurations().getIntermediate();
            case OLD -> resp.getConfigurations().getOld();
